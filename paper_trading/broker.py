...
class PaperBroker:
    ...
    def __init__(self):
        ...
        self.journal = PersistentTradeJournal()
        self.performance_engine = PerformanceEngine()
        self.performance_statistics = self.performance_engine.analyze(self.journal.records)
    ...
    def execute(self, decision):
        ...
        for pos in self.portfolio.open_positions:
            if not pos.closed and pos.order.strike == trade.strike and pos.order.option_type == trade.option_type:
    ...
    def close_position(self, position, exit_price, reason="MANUAL"):
        ...
        self.portfolio.unrealized_pnl = sum(p.pnl for p in self.portfolio.open_positions if not p.closed)
