...
def build_order_intent(decision, *, source: str = "decision") -> OrderIntent | None:
    """Create a canonical executable intent for NIFTY CE/PE only."""
    ...
    if not ok:
        return None
    option_type = str(getattr(trade, "option_type", "")).upper()
    ok, _ = POLICY.validate_option(option_type)
    if not ok:
        return None
    ...
    identity = "|".join((source, POLICY.symbol, option_type, f"{strike:.8f}", action.value, str(quantity), f"{limit_price:.8f}", str(getattr(decision, "strategy_name", "")), expiry))
    ...
    return OrderIntent(
        ...
        metadata={"signal": signal, "expiry": expiry, "underlying": POLICY.symbol, "max_trade_underlying_move": POLICY.max_trade_move, "overnight_position": False},
    )
