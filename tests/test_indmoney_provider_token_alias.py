...
def test_indmoney_provider_accepts_documented_indstocks_token_alias(monkeypatch):
    monkeypatch.delenv("INDSTOCKS_API_TOKEN", raising=False)
    monkeypatch.delenv("APITOKEN", raising=False)
    monkeypatch.setenv("INDSTOCKS_TOKEN", "documented-token\n")

    provider = INDMoneyProvider()

    assert provider.token == "documented-token"
    assert provider.headers["Authorization"] == "documented-token"
