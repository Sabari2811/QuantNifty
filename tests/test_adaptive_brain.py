...
def _closed(order_id="PAPER-001", pnl=100.0):
    return SimpleNamespace(
        order=SimpleNamespace(order_id=order_id, order_time=SimpleNamespace()),
        closed=True,
        pnl=pnl,
        exit_price=110.0,
        exit_time=None,
...
def test_brain_learns_from_the_exact_entry_fingerprint(tmp_path):
    brain = AdaptiveBrain(BrainStore(tmp_path / "brain.jsonl"))
    brain.extractor = FakeExtractor()
    opened = SimpleNamespace(position=_position("PAPER-001"), last_trade=None)
    first = brain.observe(SimpleNamespace(cycle_no=1), broker=opened)
    assert first.status == "WAITING_OUTCOME"

    closed = SimpleNamespace(position=None, last_trade=_closed("PAPER-001", 125.0))
    second = brain.observe(SimpleNamespace(cycle_no=2), broker=closed)
...
def test_brain_deduplicates_resolved_trade_by_order_id(tmp_path):
    ...
    assert second.status == "WAITING_OUTCOME"
