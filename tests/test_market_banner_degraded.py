...
def test_market_banner_renders_degraded_dashboard_without_canonical_signal(monkeypatch):
    class Column:
        ...
        def warning(self, value): self.warning_values.append(value)
        def success(self, value): self.success_values.append(value)
        def error(self, value): self.error_values.append(value)
        def metric(self, label, value): self.metric_values.append((label, value))

    column_groups = iter([[Column() for _ in range(4)], [Column() for _ in range(4)], [Column() for _ in range(2)]])
    rendered_groups = []
    ...
    dashboard = SimpleNamespace(
        dealer=SimpleNamespace(dealer_gamma="POSITIVE", market_mode="RANGE", gamma_flip=None, gamma_wall=None),
        signal={}, trade_plan={}, probability={}, spot=24100.0,
    )
    render(dashboard)

    first_group, second_group, third_group = rendered_groups
    assert first_group[0].warning_values == ["UNAVAILABLE"]
    assert ("Spot", "24,100.00") in first_group[1].metric_values
