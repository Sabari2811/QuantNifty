...
class LiveEngine:
    ...
    def _is_replay(self):
        return isinstance(self.provider, SimulationProvider)

    def _is_replay_fast(self):
        return (
            self._is_replay()
            and self.provider.runtime_mode == RuntimeMode.REPLAY_FAST
        )

    def _is_replay_recompute(self):
        return (
            self._is_replay()
            and self.provider.runtime_mode == RuntimeMode.REPLAY_RECOMPUTE
        )
