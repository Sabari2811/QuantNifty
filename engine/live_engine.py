...
class LiveEngine:
    ...
    def _evaluate_position_reconciliation(self, recovery=None, reconciliation_report=None):
        if recovery is None:
            recovery = getattr(self.ctx, "position_recovery", None)
        decision = evaluate_position_reconciliation_runtime(recovery, reconciliation_report)
        self.ctx.position_reconciliation = decision
        return decision
