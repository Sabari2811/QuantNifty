...
from analytics.intelligence.synthesis.family_aggregator import FamilyEvidenceAggregator
from analytics.intelligence.synthesis.orchestration.engine import IntelligenceSynthesisEngine
from analytics.intelligence.result import DataQuality, EvidenceSummary, RegimeState, Scenario
...
class IntelligenceService:
    ...
    def __init__(self, feature_extractor: FeatureExtractor, market_memory: MarketMemory,
                 evidence_engine: EvidenceEngine, evidence_adapter: EvidenceAdapter | None = None,
                 family_aggregator: FamilyEvidenceAggregator | None = None,
                 synthesis_engine: IntelligenceSynthesisEngine | None = None) -> None:
    ...
    def _build_data_quality(runtime_context) -> DataQuality:
        ...
        if provenance is None:
            return DataQuality(score=0.0, incomplete=True, freshness_verified=False,
                                reasons=("data_provenance_missing",))
        acquisitions = tuple(item for item in (provenance.spot, provenance.option_chain, provenance.candles)
                             if item is not None)
        if not acquisitions:
            return DataQuality(score=0.0, incomplete=True, freshness_verified=False,
        ...
        return DataQuality(score=max(0.0, min(100.0, score)), stale=False, incomplete=incomplete,
                           invalid=invalid, freshness_verified=freshness_verified,
    ...
    def _build_evidence_summary(families, cross_family) -> EvidenceSummary:
        ...
        return EvidenceSummary(
            ...
            neutral_count=neutral_count, independent_count=independent_count,
            correlated_count=correlated_count,
            confluence_score=max(0.0, min(100.0, float(getattr(cross_family, "confluence_score", 0.0)))),
            conflict_score=max(0.0, min(100.0, float(getattr(cross_family, "conflict_score", 0.0)))),
        )
    ...
    def analyze(self, runtime_context) -> IntelligenceResult:
        ...
        # Canonical analytics source: typed MarketContext. EvidenceAdapter
        # retains dict support only for legacy/unit callers.
        canonical_context = getattr(runtime_context, "market_context", None)
        evidence_source = canonical_context if canonical_context is not None else getattr(runtime_context, "analytics", {})
        evidence_items = self._evidence_adapter.extract(evidence_source)
        ...
        if decision is not None:
            synthesis = self._synthesis_engine.synthesize(
                families=families, decision=decision, regime=regime_state.regime,
                regime_confidence=regime_state.confidence, transition=regime_state.transition,
            )
        ...
        return IntelligenceResult(
            record=record, evidence=historical_evidence, recommendation=recommendation,
            confidence_before=confidence_before, confidence_after=confidence_after,
            explanation=historical_evidence.explanation, timestamp=record.timestamp,
            direction=direction, conviction=max(0.0, min(100.0, conviction_score)),
            opportunity_quality=max(0.0, min(100.0, opportunity_score)),
            evidence_items=evidence_items, evidence_summary=evidence_summary,
            regime=regime_state, primary_scenario=primary_scenario,
            alternative_scenario=alternative_scenario,
            invalidation=tuple(item for item in (
                primary_scenario.invalidation if primary_scenario is not None else ""
            ).split(";") if item.strip()),
