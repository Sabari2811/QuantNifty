...
class OIFlowEngine:
    ...
    def _trend(self, ce_summary: dict, pe_summary: dict) -> str:
        total_long = ce_summary["long_buildup"] + pe_summary["long_buildup"]
        total_cover = ce_summary["short_covering"] + pe_summary["short_covering"]
    ...
    def _prepare_previous(self, previous_greeks_df: pd.DataFrame) -> pd.DataFrame:
        previous = previous_greeks_df.copy()
        if "Strike" not in previous.columns and "STRIKE" in previous.columns:
            previous = previous.rename(columns={"STRIKE": "Strike"})
        if "Strike" not in previous.columns:
            return pd.DataFrame()
        previous["Strike"] = pd.to_numeric(previous["Strike"], errors="coerce")
        previous = previous.dropna(subset=["Strike"])
        required = ["CE_LTP", "CE_OI", "PE_LTP", "PE_OI"]
        for column in required:
            if column not in previous.columns:
                previous[column] = 0.0
            previous[column] = self._numeric(previous, column)
        return previous[["Strike", "CE_LTP", "CE_OI", "PE_LTP", "PE_OI"]].copy()

    def _first_snapshot(self, df: pd.DataFrame) -> dict:
        df["CE_FLOW"] = "UNKNOWN"
        df["PE_FLOW"] = "UNKNOWN"
    ...
    def _calculate_deltas(self, current: pd.DataFrame, previous: pd.DataFrame) -> pd.DataFrame:
        if previous.empty:
            return current
    ...
    def analyze(self, greeks_df: pd.DataFrame, previous_greeks_df: pd.DataFrame | None = None):
        if greeks_df is None or greeks_df.empty:
            return {"summary": {}, "table": greeks_df}
        df = greeks_df.copy()
        if "Strike" not in df.columns and "STRIKE" in df.columns:
            df = df.rename(columns={"STRIKE": "Strike"})
        if "Strike" not in df.columns:
            df["Strike"] = range(len(df))
        df["Strike"] = pd.to_numeric(df["Strike"], errors="coerce")
        for column in ["CE_LTP", "CE_OI", "PE_LTP", "PE_OI"]:
            df[column] = self._numeric(df, column)
        if previous_greeks_df is None or previous_greeks_df.empty:
            return self._first_snapshot(df)
        previous = self._prepare_previous(previous_greeks_df)
        if previous.empty:
            return self._first_snapshot(df)
        ...
        summary = {
            ...
            "call": ce_summary,
            "put": pe_summary,
            "market_bias": self._market_bias(ce_summary, pe_summary),
            "trend": self._trend(ce_summary, pe_summary),
            "total_strikes": len(df),
        }
        return {"summary": summary, "table": df}
